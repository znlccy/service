  ...
  var Accordion = function (obj, option) {
    ...
    $.each(this.defaults.imageObj, function (i, src) {
      ...
      $(img)
        .on('load error', function () {
          self.winWidth = self.obj.find('ul[data-name="accordion"]')
            .innerWidth();
        });
    });

    this.winWidth = this.obj.find('ul[data-name="accordion"]')
      .innerWidth();
    ...
    if (this.defaults.arrangement === 'horizontal') {
      this.obj.hover(function () {
        $(this)
          .find('.accordion-li')
          .css('transition', 'width ' + self.defaults.animateTime + '');
      });
      var response = (this.defaults.response) ? self.eachImgWidth : self.defaults.defaultWidth;
      this.li.hover(function (e) {
        $(this)
          .siblings('.accordion-li')
          .width(self.eachSmallImgWidth);
        $(this)
          .addClass('active')
          .css('width', self.defaults.animateWidth);
        if (self.LessThanIE8) {
          $(this)
            .find('.accordion-description')
            .css('background', self.defaults.styleColor.background)
            .parents('.accordion-li')
            .siblings()
            .find('.accordion-description')
            .css('background', self.defaults.styleColor.background);
        } else {
          $(this)
            .find('.accordion-description')
            .css('background', 'linear-gradient(' + self.colorMD5Count(self.defaults.styleColor.background, 0).rgba + ',' + self.defaults.styleColor.background + ')');
        }
      }, function (e) {
        $(this)
          .removeClass('active');
        ...
        $(this)
          .find('.accordion-description')
          .css('background', 'linear-gradient(' + self.defaults.styleColor.background + ',' + self.defaults.styleColor.background + ')');
      ...
      //     .hasClass('active')) {
      //     $(this)
      //       .css('height', self.defaults.defaultHeight)
      //       .removeClass('active');
      //     $(this)
      //       .find('.accordion-description')
      //       .css({
      ...
      //       .removeClass('active');
      //     $(this)
      //       .css('height', self.defaults.animateHeight);
      //     $(this)
      //       .find('.accordion-description')
      //       .css({
  ...
  Accordion.prototype = {
    defaultDOM: function () {
      ...
      $.each(imgArr, function (index, val) {
        ...
        liHtml +=
          ...
          '<span class="accordion-icon">' + val.icon + '</span>' +
          '<img class="accordion-img" src="' + val.src + '" alt="' + val.alt + '" width="640"height="480"/>' +
          '</a>' +
          '<div class="accordion-description">' +
          '<h4 class="accordion-title">' + val.title + '</h4>' +
          '<p class="accordion-content"style="padding-bottom: 20PX">' + val.content + '</p>' +
          '</div>' +
          '</li>';
      });
    ...
    cutOverflowString: function () {
      ...
      this.content.each(function (index, el) {
        ...
        if (len > _this_.defaults.fontCut) {
          var sub = $(el)
            .text()
            .substring(0, _this_.defaults.fontCut);
          $(el)
            .text(sub + '...');
        }
      });
    ...
    responseDOM: function () {
      if (this.defaults.arrangement === 'verticality') {
        ...
        this.obj.find('ul[data-name="accordion"]')
          .width(this.winWidth + this.winWidth / 95);
      } else {
