...
use think\Request;
use app\admin\model\EnterTeam as EnterTeamModel;

class EnterTeam extends BaseController
{
    /**
     * 显示资源列表
     *
    ...
    public function delete()
    {
        ...
        $result   = $this->validate($data, 'EnterTeam.delete');
        if (true !== $result) {
            return json(['code' => 401, 'message' => $result]);
        }
        $result = EnterTeamModel::destroy($id);
        if ($result) {
    ...
    {
        $team = EnterTeamModel::field('id,company')->select();
        if (!empty($team)) {
