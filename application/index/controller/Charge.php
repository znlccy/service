...
use app\index\model\Incidental;
...
//        Hook::listen('index_cross');
    ...
    public function index()
    {
        ...
        $enter_team_id = session('admin.enter_team_id');
        // 对于UNION查询以及一些特殊的复杂查询，推荐使用这种方式首先单独查询总记录数，然后再传入分页方法
        // 获取已签署合同的销售订单总数
        $sale_order_ids = Contract::where(['status' => 2])->column('order_id');
        $sale_order_ids = Order::whereIn('id', $sale_order_ids)->where('team_id',$enter_team_id)->colum('id');
        $rec_order_count = Db::table('tb_rec_order')->whereIn('sale_order_id', $sale_order_ids)->count();
        $incidental_ids = IncidentalOrder::where(['status' => 2])->column('id');
        $inc_order_count = Db::table('tb_incidental_order')->where(['incidental_id' => $incidental_ids, 'team_id' => $enter_team_id])->count();
        $total_count = intval($rec_order_count + $inc_order_count);
        $order = Db::field('id,order_no,price,status,create_time')
            ->where($conditions)
            ->whereIn('sale_order_id', $sale_order_ids)
            ->table('tb_rec_order')
            ->union(function($query) use ($conditions, $incidental_ids, $enter_team_id) {
                $query->field('id,order_no,price,status,create_time')
                    ->where($conditions)
                    ->where(['incidental_id' => $incidental_ids, 'team_id' => $enter_team_id])
                    ->table('tb_incidental_order');
